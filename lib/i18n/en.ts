export const en = {
  common: {
    languageName: "English",
    bookNow: "Book Now",
    scrollToExplore: "Scroll to explore",
    getQuote: "Non-binding estimate",
    callNow: "Call now",
    whatsappAriaLabel: "Message us on WhatsApp",
  },
  cookieConsent: {
    message:
      "We use cookies and measure how you use the site – including session recordings – to see where visitors leave and what does not work. Details:",
    decline: "Decline",
    accept: "Accept",
  },
  welcomePromo: {
    badge: "Limited offer • New clients only",
    eyebrow: "Welcome to Crystal Detailing",
    discount: "−10%",
    discountSubtitle: "on your first detail",
    body: "Premium mobile detailing at your location—home, office, or wherever you need us. The discount applies on your first booking through our online system.",
    cta: "Book my first detail",
    dismiss: "No thanks, I'll pay full price",
    closeLabel: "Close offer",
  },
  nav: {
    brandTitle: "Mobile Car Detailing Bratislava",
    logoAlt: "Crystal Detailing – mobile car detailing Bratislava",
    homeAriaLabel: "Crystal Detailing – mobile car detailing Bratislava, home",
    aboutUs: "About Us",
    services: "Services",
    servicesMenu: "Services menu",
    forBusiness: "For Business",
    pricing: "Price list",
    howItWorks: "How\u00A0It\u00A0Works",
    locations: "Locations",
    blog: "Blog",
    reviews: "Reviews",
    contact: "Contact",
  },
  cennikPage: {
    h1: "Service price list",
    intro:
      "Overview of Crystal Detailing packages and add-on services. Package prices vary by vehicle size—select a category above. Get an exact quote in our calculator.",
    packagesHeading: "Packages",
    addonsHeading: "Add-on services",
    backHome: "Back to home",
  },
  seo: {
    homeTitle: "Mobile Car Detailing Bratislava | Crystal Detailing",
    homeDescription:
      "Mobile interior and exterior car cleaning in Bratislava. We come to your home or office with our own water and power.",
    breadcrumbAria: "Page navigation",
    breadcrumbHome: "Home",
    breadcrumbServices: "Services",
    breadcrumbPricing: "Price list",
  },
  servicePage: {
    highlightsTitle: "Key benefits",
    includesTitle: "What is included",
    relatedTitle: "Related pages",
    bookNow: "Book now",
    getQuote: "Price calculator",
    callNow: "Call now",
    whatsappAriaLabel: "Message us on WhatsApp",
  },
  hero: {
    promoBadge: "Summer sale −10% on all packages",
    availabilityBadge: {
      title: "Online and available 24/7",
    },
    title: "Mobile car cleaning at your home",
    titleHighlight: "",
    subtitle:
      "We come to your home or office. No driving, traffic jams, or need to hook up water or electricity.",
    description:
      "Experience the pinnacle of automotive care without leaving your home or office. Our expert detailers bring premium products and equipment directly to you, transforming your vehicle into a masterpiece of automotive perfection.",
    availableIn: "Available in",
    andSurrounding: "and surrounding areas",
    stats: [
      { value: "600+", label: "detailed vehicles" },
      { value: "5.0", label: "average rating", showStar: true },
      { value: "4", label: "years of experience" },
      { value: "2000+", label: "hours of time saved" },
    ],
  },
  heroLead: {
    badge: "We provide service 7 days a week",
    title: "Mobile car cleaning at your place in Bratislava.",
    subtitle:
      "We come to your home or office. No driving, traffic jams, or need to hook up water or electricity.",
    socialProof: "More than 50+ happy customers",
    socialProofAria: "Rating 5 out of 5 stars",
    trust1: "We cover all of Bratislava",
    trust2: "We come to your home or office",
    trust3: "No water or electricity needed",
    formTitle: "Free quote",
    formSubtitle: "Fill in the form for a no-obligation price estimate for mobile detailing.",
    nameLabel: "Name",
    namePlaceholder: "Enter your name",
    phoneLabel: "Phone number",
    phonePlaceholder: "Enter your phone number",
    emailLabel: "Email",
    emailPlaceholder: "Enter your email",
    messageLabel: "Tell us what you need",
    messagePlaceholder: "e.g. interior cleaning, seat shampooing",
    cityLabel: "Location",
    citySelectedPrefix: "Selected city:",
    otherCity: "Other / not listed",
    discountLabel: "I have a discount code",
    discountPlaceholder: "Enter the code",
    consentBeforeLink: "By submitting this form I agree to the processing of personal data according to the",
    consentLink: "privacy policy",
    consentRequired: "Please confirm consent before sending.",
    submit: "Get a free quote",
    submitting: "Sending...",
    submitNote: "After you send the form we will contact you within 24 hours.",
    successTitle: "Thank you, inquiry sent.",
    successText: "We will get back to you within 24 hours with a price and the next available slot.",
    errorGeneric: "Something went wrong. Please try again.",
  },
  handoverShowcase: {
    title: "We hand your car back not just clean—but like new",
    beforeLabel: "Before",
    afterLabel: "After",
    dragHint: "Drag or tap the photo to compare",
    decorativeBackgroundAlt: "Clean car after Crystal Detailing mobile service",
    beforeImageAlt: "Car before Crystal Detailing mobile service",
    afterImageAlt: "Car after Crystal Detailing mobile service",
    sameResultCta: "I want the same result",
  },
  about: {
    title: "About Crystal Detailing",
    paragraph1:
      "Founded in 2023, Crystal Detailing is Slovakia's pioneering mobile car detailing service, bringing premium automotive care directly to your doorstep. We combine cutting-edge techniques with luxury products to deliver an unparalleled detailing experience.",
    paragraph2:
      "Our team consists of certified detailing professionals with years of experience working with high-end vehicles. We understand that your car is more than just transportation—it's an extension of your personal style and success.",
    paragraph3:
      "What sets us apart is our unwavering commitment to excellence, our use of premium products, and our ability to provide showroom-quality results without you having to leave your home or office. We respect your time as much as we respect your vehicle.",
    fullyInsured: "Fully Insured",
    premiumProducts: "Premium Products",
    certifiedDetailers: "Certified Detailers",
  },
  whyChooseUs: {
    title: "Why Choose Us",
    imageAlt: "Crystal Detailing team member in the detailing studio",
    items: [
      {
        title: "We come to you—you don't have to go anywhere",
        description:
          "We save you time and hassle. No waiting in line, no driving to a car wash, no planning your day around cleaning the car. We come straight to your home, workplace, or agreed location and take care of everything.",
      },
      {
        title: "Precise work and experience you can rely on",
        description:
          "We clean every car thoroughly with attention to detail. Our experience tells us which methods, products, and techniques to use on different interiors, paintwork, and levels of soiling so the result looks clean, fresh, and professional.",
      },
      {
        title: "Your car will feel like new again",
        description:
          "This is more than a regular wash. We focus on the overall feel of your car—a clean interior, fresh scent, removed stains, dust, dirt, and neglected details. The result is a car you'll be happy to get back into.",
      },
    ],
  },
  howItWorks: {
    title: "How It Works",
    subtitle: "Our mobile detailing service brings the premium car wash experience directly to you",
    step1Title: "1. Book an Appointment",
    step1Description:
      "Select your preferred package and schedule a convenient time through our online booking system. We offer flexible scheduling options to fit your busy lifestyle.",
    step2Title: "2. We Come to You",
    step2Description:
      "Our fully-equipped mobile detailing unit arrives at your location, whether home or office. We bring our own water, electricity, and premium detailing products.",
    step3Title: "3. Enjoy the Results",
    step3Description:
      "Relax while our professional detailers transform your vehicle to showroom condition. When we're done, you'll receive a walkthrough of all the work completed.",
    bookAppointment: "Book Your Appointment",
  },
  mobileService: {
    title: "We Are Crystal Detailing",
    shortLead:
      "Premium mobile detailing for those who expect perfection in every detail.",
    description:
      "Our fully equipped mobile detailing unit brings the premium car wash experience directly to your location. No need to drive anywhere or wait in line - we handle everything while you focus on what matters to you.",
    availability: "Available 7 days a week",
  },
  services: {
    title: "Our Premium Packages",
    subtitle:
      "Choose from our carefully crafted detailing packages designed to restore and protect your vehicle",
    travelNote: {
      baPrefix: "Bratislava: dispatch",
      free: "free",
      within15kmPrefix: "within 15 km of BA",
      within15kmFee: "+€10",
      from15kmPrefix: "from 15 km from BA",
      perKmFee: "€0.50/km",
      petHairLabel: "Pet hair",
      petHairFee: "+€20",
      extraDirtyLabel: "Excessive contamination",
      extraDirtyFee: "20% of the price",
    },
    mostPopularLabel: "Most popular",
    lastAvailableSlotsBadge: "Last available slots",
    essential: {
      title: "Interior Package",
      subtitle: "Ideal for regular interior maintenance",
      price: "€79",
      features: [
        "Hand interior cleaning",
        "Screen and surface cleaning",
        "Interior vacuuming and dusting",
        "Trunk cleaning",
        "Dashboard and console wipe-down",
        "Seat treatment / impregnation",
        "Window cleaning",
      ],
      selectPackage: "Select Package",
    },
    premium: {
      title: "Exterior Package",
      subtitle: "Complete exterior care",
      price: "€89",
      mostPopular: "Popular",
      features: [
        "Full exterior vehicle cleaning",
        "Bio wax application",
        "Wheel cleaning",
        "Complete glass cleaning",
        "Insect removal",
        "Plastic and gloss part restoration",
      ],
      selectPackage: "Select Package",
    },
    ultimate: {
      title: "Complete Package",
      subtitle: "Full luxury experience for both interior and exterior",
      price: "€149",
      features: [
        "Everything in the Interior Package",
        "Everything in the Exterior Package",
        "Interior impregnation",
      ],
      selectPackage: "Select Package",
    },
  },
  additionalServices: {
    title: "Additional Services",
    subtitle: "Customize your detailing experience with our specialized services",
    service: "Service",
    description: "Description",
    price: "Price",
    items: [
      {
        name: "Deep Seat Cleaning",
        description: "Professional extraction and deep cleaning of all seats and upholstery",
        price: "€35",
      },
      {
        name: "Ozone Treatment",
        description: "Eliminate odors and sanitize your vehicle's interior with ozone technology",
        price: "€45",
      },
      {
        name: "Headlight Restoration",
        description: "Restore cloudy, yellowed headlights to crystal clear condition",
        price: "€40",
      },
      {
        name: "Leather Treatment",
        description: "Deep clean, condition and protect leather surfaces",
        price: "€55",
      },
      {
        name: "Engine Bay Detailing",
        description: "Clean and dress your engine bay for a showroom appearance",
        price: "€60",
      },
    ],
    note: "Additional services can be booked on their own or added to any package. The final price may vary depending on vehicle condition.",
    new: "New",
  },
  headlightRenewal: {
    title: "Headlight Renewal",
    subtitle: "Restore the crystal clear appearance of your headlights",
    description: "Our professional headlight renewal service transforms cloudy and yellowed headlights back to their original crystal clear condition. We use specialized techniques and premium products to restore clarity and brightness.",
    beforeAfter: "Before and After",
    bookService: "Book Service",
    card: {
      title: "Headlight Restoration",
      subtitle: "Full restoration of both headlights",
      features: [
        "Removal of oxidation and yellowing",
        "Headlight sanding",
        "Headlight polishing",
        "UV protection coating",
      ],
      price: "70 €",
      priceNote: "for both headlights",
      selectPackage: "Select Service",
    },
  },
  whatToExpect: {
    title: "What to Expect",
    subtitle:
      "When you choose Crystal Detailing, you're not just getting a car wash—you're investing in a premium experience tailored to your vehicle's specific needs",
    consultation: {
      title: "Professional Consultation",
      description:
        "Our process begins with a thorough assessment of your vehicle's condition. We'll discuss your concerns and goals, recommending the most appropriate treatments for your specific needs.",
    },
    attention: {
      title: "Meticulous Attention to Detail",
      description:
        "We don't rush the process. Each vehicle receives careful attention, with our detailers focusing on every curve, crevice, and surface to ensure immaculate results that exceed expectations.",
    },
    products: {
      title: "Premium Products & Equipment",
      description:
        "We use only the highest quality detailing products and equipment, including ceramic coatings, professional-grade polishers, and pH-balanced cleaners that are gentle yet effective on all surfaces.",
    },
    convenience: {
      title: "Convenience & Flexibility",
      description:
        "We work around your schedule, not the other way around. Our mobile service means you can continue with your day while we transform your vehicle at your home, office, or location of choice.",
    },
    transparency: {
      title: "Transparent Process",
      description:
        "We believe in complete transparency. Our detailers will walk you through each step of the process, explaining the products used and the techniques applied to achieve the best results.",
    },
    results: {
      title: "Lasting Results",
      description:
        "Our detailing services don't just clean—they protect. From ceramic coatings to paint sealants, we provide treatments that extend the life and beauty of your vehicle long after we leave.",
    },
  },
  mediaBento: {
    title: "Results in Detail",
    subtitle: "See more of our work on our Instagram.",
    instagramCta: "Follow us on Instagram",
    labels: {
      mobileStudio: "Mobile studio in action",
    },
    photoAlts: [
      "Mobile Porsche Cayenne detailing at the customer's home",
      "Detailed interior cleaning after mobile detailing",
      "Crystal mobile detailing unit at work on site",
      "Luxury vehicle exterior after professional cleaning",
      "Professional care for vehicle interior surfaces",
      "SUV cargo area cleaning with rear seats folded down",
      "Detailed cleaning of the rear interior of a Porsche Cayenne",
    ],
    videoFallbackAlt: "Crystal Detailing mobile studio working on a customer car",
    showMore: "Show more",
    showLess: "Show less",
  },
  reviews: {
    title: "More than 50 customers have given us 5 stars",
    subtitle:
      "Real Google reviews from customers whose cars we cleaned at home or at work in Bratislava.",
    googleRatingValue: "5.0",
    googleReviewCount: "from 50+ reviews",
    googleRatingAria: "Rating 5.0 out of 5 stars",
    viewAllGoogleReviews: "View all reviews on Google",
    previousReview: "Previous review",
    nextReview: "Next review",
    avatarAlt: "Satisfied Crystal Detailing customer",
  },
  faq: {
    title: "Frequently Asked Questions",
    subtitle: "Everything important about our mobile detailing in one place",
    items: [
      {
        question: "Do I need to be present during detailing?",
        answer:
          "No, you do not. You only need to provide access to the vehicle and we will take care of everything else. After completion, we will inform you and hand the vehicle back to you like new.",
      },
      {
        question: "Is any wastewater or waste produced during cleaning?",
        answer:
          "No. Our process does not create wastewater and no dirt is discharged into the surroundings. We use waterless technology that is environmentally friendly. All dirt and waste are captured and then taken away by us.",
      },
      {
        question: "Do you need access to electricity or water?",
        answer:
          "No, we provide everything ourselves. We have our own water tank and power source, so we can work anywhere - in residential areas, under your office, in front of a gym, and more.",
      },
      {
        question: "Where does mobile detailing take place?",
        answer:
          "We detail your car directly at your home, workplace, or anywhere there is access to the vehicle. You do not need to go anywhere or move your car. You save both time and hassle.",
      },
      {
        question: "Is mobile detailing safe for my car?",
        answer:
          "Yes. We use professional car care products and gentle procedures that protect both your paint and your interior.",
      },
      {
        question: "What if my car is full of personal items?",
        answer:
          "You do not need to worry about anything. We remove all items from the car and after detailing we carefully put them back in their place.",
      },
    ],
  },
  booking: {
    title: "Ready to Experience Premium Mobile Detailing?",
    description:
      "Book your appointment now through our online system and transform your vehicle with our luxury detailing service. We service all of Slovakia, with special focus on Bratislava, Košice, and surrounding areas.",
    bookAppointment: "Book Your Appointment",
  },
  contactForm: {
    title: "Write to us",
    subtitle: "Have a question or a specific request? Send a message and we'll get back to you.",
    nameLabel: "Name",
    namePlaceholder: "Your name",
    companyLabel: "Company name",
    companyPlaceholder: "Your company",
    optional: "optional",
    emailLabel: "Contact email",
    phoneLabel: "Contact phone",
    messageLabel: "Message",
    messagePlaceholder: "Briefly tell us what you need...",
    submit: "Send",
    submitting: "Sending...",
    successTitle: "Thanks, message sent.",
    successText: "We'll get back to you shortly — usually within one business day.",
    errorGeneric: "Something went wrong. Please try again.",
    consent: "By submitting you agree to the processing of your contact details to handle the inquiry.",
  },
  businessContactForm: {
    companyLabel: "Company name",
    companyPlaceholder: "Your company name",
    industryLabel: "Industry or sector",
    industryPlaceholder: "e.g. passenger transport, construction, hospitality...",
    nameLabel: "Contact person",
    namePlaceholder: "Full name",
    fleetLabel: "Number of vehicles in fleet",
    fleetPlaceholder: "e.g. 12",
    emailLabel: "Contact email",
    phoneLabel: "Contact phone",
    messageLabel: "Note",
    messagePlaceholder: "Vehicle types, care frequency, preferred schedule...",
    optional: "optional",
    submit: "Send enquiry",
    submitting: "Sending...",
    successTitle: "Thanks, enquiry sent.",
    successText: "We'll prepare a business quote and get back to you shortly—usually within one business day.",
    errorGeneric: "Something went wrong. Please try again.",
    consent: "By submitting you agree to the processing of your details to handle the business enquiry.",
  },
  blog: {
    title: "Blog",
    subtitle: "Tips, advice, and news from mobile car detailing",
    pageTitle: "Crystal Detailing Blog",
    pageIntro:
      "Practical articles on mobile detailing, upholstery cleaning, headlight restoration, and interior care in Bratislava and beyond.",
    viewAll: "All articles",
    readMore: "Read more",
    backHome: "Back to home",
    backToBlog: "Back to blog",
    sidebarCategories: "Categories",
    allCategories: "All articles",
    noPosts: "No articles in this category yet.",
    sortLabel: "Sort",
    sortNewest: "Newest first",
    sortOldest: "Oldest first",
    loading: "Loading…",
    readingTimeOne: "1 min read",
    readingTimeMany: "{minutes} min read",
  },
  footer: {
    tagline: "Slovakia's first luxury mobile car detailing service that comes to your location.",
    contactHeading: "Contact",
    quickLinksHeading: "Quick Links",
    locations: "Locations",
    blog: "Blog",
    priceCalculator: "Price Calculator",
    serviceMobilnyDetailing: "Mobile detailing",
    serviceTepovanie: "Car upholstery cleaning",
    serviceRenovaciaSvetlometov: "Headlight restoration",
    privacyPolicy: "Privacy Policy",
    termsOfService: "Terms of Service",
    contactUs: "Contact Us",
    allRightsReserved: "All rights reserved.",
  },
  languageSwitcher: {
    changeLanguage: "Change language",
  },
  legal: {
    lastUpdated: "Last updated:",
  },
  carSizes: {
    small: "Small",
    smallDesc: "Hatchback / Sedan",
    suv: "Medium",
    suvDesc: "Estate / Small SUV",
    van: "Large",
    vanDesc: "Large SUV / Van",
  },
  gallery: {
    title: "Customer Showcase",
    subtitle: "Explore our portfolio of premium detailing work on customer vehicles",
    luxurySedanTitle: "Luxury Sedan Transformation",
    suvInteriorTitle: "Premium SUV Interior",
    sportsCarTitle: "Ceramic Coating Excellence",
    exoticCarTitle: "Exotic Car Perfection",
    classicCarTitle: "Classic Beauty Restored",
    paintCorrectionTitle: "Flawless Paint Correction",
  },
  calculator: {
    title: "Get Your Instant Quote",
    subtitle: "Answer a few quick questions to get your personalized estimate",
    step: "Step",
    of: "of",
    back: "Back",
    next: "Next",
    calculating: "Calculating...",
    almostDone: "You're almost done (2 steps left)",
    mostPopular: "Most popular",
    step1Title: "What type of vehicle do you have?",
    vehicleSmall: "Small",
    vehicleSmallDesc: "Hatchback / Sedan",
    vehicleMedium: "Medium",
    vehicleMediumDesc: "Estate / Small SUV",
    vehicleLarge: "Large",
    vehicleLargeDesc: "Large SUV / Van",
    step2Title: "What's the condition of your vehicle?",
    conditionNormal: "Normal",
    conditionNormalDesc: "Regular maintenance",
    conditionDirty: "Dirty",
    conditionDirtyDesc: "Needs extra attention",
    conditionExtreme: "Extreme",
    conditionExtremeDesc: "Heavy soiling",
    step3Title: "What service do you need?",
    serviceExterior: "Exterior",
    serviceExteriorDesc: "Outside cleaning & protection",
    serviceInterior: "Interior",
    serviceInteriorDesc: "Inside cleaning & care",
    serviceBoth: "Exterior + Interior",
    serviceBothDesc: "Complete detailing",
    serviceSeats: "Seats Only",
    serviceSeatsDesc: "Seat shampooing",
    step4Title: "Interested in any add-on services? (optional)",
    step4Subtitle:
      "Selected services are priced individually on-site — they are not included in the price above.",
    addonSeats: "Seat shampooing",
    addonSeatsDesc: "Deep extraction of fabric seats.",
    addonCarpets: "Carpet shampooing",
    addonCarpetsDesc: "Shampooing of floor carpets and mats.",
    addonLeather: "Leather seat & trim conditioning",
    addonLeatherDesc: "Cleaning and conditioning of leather surfaces.",
    addonHeadlights: "Headlight restoration",
    addonHeadlightsDesc: "Restoration of cloudy and yellowed headlights.",
    addonEngine: "Engine bay cleaning",
    addonEngineDesc: "Cleaning and detailing of the engine compartment – €60.",
    addonNeedVehicle: "Go back to step 1 and select a vehicle type to see add-on prices.",
    priceBreakdownTitle: "How the price is calculated",
    priceBreakdownPackage: "Package",
    priceBreakdownLargeVehicle: "Large vehicle surcharge",
    priceBreakdownExtremeCondition: "Extreme dirt surcharge",
    priceBreakdownTotal: "Total",
    priceBreakdownAddonsNote: "Add-on services are priced individually on-site.",
    leadTitle: "Want a no-obligation quote?",
    leadSubtitle: "Leave us your details and we'll get back to you with an exact price and time slot.",
    leadPhoneLabel: "Phone",
    leadEmailLabel: "Email",
    leadSubmit: "Send no-obligation inquiry",
    leadSubmitting: "Sending...",
    leadConsent: "By submitting you agree to the processing of your contact details to handle the inquiry.",
    leadSuccessTitle: "Thanks, we've received your inquiry.",
    leadSuccessText: "We'll get in touch shortly with a no-obligation quote using the contacts you provided.",
    leadErrorGeneric: "Something went wrong. Please try again.",
    step5Title: "Where should we come?",
    cityLabel: "City or ZIP code",
    cityPlaceholder: "Bratislava, 81101...",
    parkingLabel: "Parking type",
    parkingPrivate: "Private",
    parkingPrivateDesc: "House / Company",
    parkingPublic: "Public",
    parkingPublicDesc: "Street / Public parking",
    backToHome: "Return to main page",
    resultTitle: "Your Estimate is Ready!",
    estimatedPrice: "Estimated Price",
    estimatedTime: "Estimated Time",
    disclaimer: "Final price may slightly vary after on-site inspection.",
    trustCopy: "No surprises – final price confirmed before we start.",
    bookTime: "Book a Time",
    contactUs: "Contact Us / WhatsApp",
    teamSignature: "— Crystal Detailing Team",
    h: "h",
    min: "min",
    whatsappMessage: "Hi! I'd like to book a detailing service. Estimated price: {price} €, Estimated time: {time}",
  },
  bookingFlow: {
    title: "Book an appointment",
    subtitle: "Choose your vehicle, service and a free slot. We will confirm the booking.",
    step: "Step",
    of: "of",
    back: "Back",
    next: "Next",
    submit: "Send booking",
    submitting: "Sending...",
    stepVehicle: "What is your vehicle size?",
    vehicleSmall: "Small",
    vehicleSmallDesc: "Hatchback / Sedan",
    vehicleMedium: "Medium",
    vehicleMediumDesc: "Estate / Small SUV",
    vehicleLarge: "Large",
    vehicleLargeDesc: "Large SUV / Van",
    stepService: "Which service do you need?",
    serviceTepovanie: "Upholstery cleaning",
    serviceTepovanieDesc: "Standalone professional interior shampooing",
    stepExtras: "Anything extra?",
    stepExtrasSubtitle: "Optional add-ons. Upholstery appears if you did not choose it as the main service.",
    extraTepovanie: "Interior upholstery cleaning",
    extraTepovanieDesc: "Deep shampooing of seats, carpets and textiles.",
    extraClimate: "Air-conditioning clean",
    extraClimateDesc: "Disinfection and refresh of the AC system.",
    extraPlastics: "Plastic impregnation",
    extraPlasticsDesc: "Treatment and protection of interior plastics.",
    extraLeather: "Leather impregnation",
    extraLeatherDesc: "Cleaning and conditioning of leather surfaces.",
    extraHeadlights: "Headlight restoration",
    extraHeadlightsDesc: "Restore cloudy or yellowed headlights.",
    extraEngine: "Engine bay cleaning",
    extraEngineDesc: "Gentle cleaning of the engine bay.",
    extrasNone: "No add-ons",
    stepAddress: "Where should we come?",
    addressLabel: "Address",
    addressPlaceholder: "Street, number, city",
    addressHint: "We come to your home or office with our own water and power.",
    nameLabel: "Full name",
    phoneLabel: "Phone",
    emailLabel: "Email",
    notesLabel: "Note (optional)",
    notesPlaceholder: "Car type, parking, entrance…",
    stepWhen: "Choose a date",
    monthPrev: "Previous month",
    monthNext: "Next month",
    noSlots: "No free times left on this day.",
    pickDay: "Select a day in the calendar.",
    legendAvailable: "Fully free",
    legendPartial: "Slots still free",
    legendFull: "Fully booked",
    legendClosed: "Closed",
    estimate: "Estimated price",
    estimateNote: "Final price is confirmed before we start.",
    successTitle: "Thank you, booking sent",
    successText:
      "We will review your booking now. The confirmation email arrives only after we approve the appointment.",
    successBack: "Back to home",
    errorGeneric: "The booking could not be sent. Please try again.",
    consent: "By sending you agree to the processing of your data to handle the booking.",
  },
}
